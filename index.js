...
const checkIndex = (arrayLength) => {
  ...
  return index >= arrayLength
    ? setTimeout(
        () =>
          (wrapper.innerHTML = `<h1>You have completed the Quiz with ${correctNumber} points!</h1>`),
        500
      )
    : false;
};
...
let setQuestions = () => {
  ...
  let questionsArr = [correct_answer, ...incorrect_answers].sort(
    () => Math.random() - 0.5
  );
